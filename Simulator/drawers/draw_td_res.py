...
model_type = 'fullnet'
file_path = f'{PROJECT_ROOT}\\results\\ds_proj\\td_results\\TD实验结果.xlsx'
df = pd.read_excel(file_path, sheet_name=model_type)

file_path = f'{PROJECT_ROOT}\\results\\ds_proj_original\\td_results\\TD实验结果.xlsx'
df_original = pd.read_excel(file_path, sheet_name=model_type)

file_path = f'{PROJECT_ROOT}\\results\\ds_proj\\td_results\\vertex结果_规定变化根节点电压.xlsx'
