...
# 定义范围和步长
x = np.linspace(-0.25, 0.25, num=21)  # 在[-0.2, 0.2]内生成20个点
y = np.linspace(-0.25, 0.25, num=21)
# 生成网格点
X, Y = np.meshgrid(x, y)
...
def gen_error_data(points, filepath = None):
    ...
    for dtheta in points:
        A_pred, b_pred = fullnet(torch.tensor(dtheta,dtype=torch.float32))
        A_pred = A_pred[0].detach().cpu().numpy()
        b_pred = b_pred[0].detach().cpu().numpy()
        ec.update_parameters({'theta':np.array([0.7,0.7])+dtheta})
    ...
    error_total_mean =  [(feas + opt)/2 for feas, opt in zip(error_feas_mean, error_opt_mean)]
    # 创建DataFrame
    df = pd.DataFrame({
