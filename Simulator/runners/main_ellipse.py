"""Train the ellipse Extended Data experiment."""
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Train PolyFormer on the two-dimensional ellipse experiment."
    )
    parser.add_argument(
        "--model-type",
...
def build_model(case, model_type, device):
    ...
    if not pretrained_path.is_file():
        raise FileNotFoundError(
            f"Pretrained ellipse weights are required for {model_type}: {pretrained_path}"
        )
...
def build_case(case_options, smoke):
    if not smoke:
        return case_ellipse(**case_options)
...
def run(args):
    ...
    n_train = 1 if args.smoke else (300 if args.model_type == "pretrainnet" else 15)
    trainer.train(
        n_train=n_train,
    ...
    if args.smoke or args.no_save:
        print("Ellipse run completed; weights were not saved.")
    else:
        output_path = Path(case["result_path"])
        output_path.parent.mkdir(parents=True, exist_ok=True)
        torch.save(model.state_dict(), output_path)
        print(f"Saved ellipse weights to {output_path}")
    return trainer
