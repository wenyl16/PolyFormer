...
for casename, ppc in dscases.items():
    ...
    case = TD_case.DScase_train(casedata=ppc, model_type = model_type, device=device, plot_flag=False, noise_range=[-0.04,0.04])
    ...
    trainer.initialize()
    trainer.train(n_train = n_train*3 , params_data=case['params'], parallel= parallel)
    trainer.configure(rate_opt_feas = 1e-4)
    trainer.initialize()
    trainer.train(n_train = 2 , params_data=case['params'], parallel= parallel)
