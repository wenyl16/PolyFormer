...
mpc = MPCcase()
case_fr = mpc.build_simplecase_fr(model_type=model_type)


pretrainnet =  PreTrainNet(case_fr['A_hat'],case_fr['b_hat'],is_epigraph=False)
pretrainnet.load_state_dict(torch.load(f'{PROJECT_ROOT}\\results\\MPC\\{case_fr['casename']}\\pretrainnet_weights_fr.pth', map_location=device))
...
# trainer.configure(rate_opt_feas = 0.)
