...
from Simulator.testers.testing import case17me_ds
from Simulator_v5.cases.TD_case import case33bw_ds
...
def case533mt_hi_ds(root_voltage = 1.0, radial = True, flex_percent = 0.5, flex_rate = 0.3):
    ...
    data = loadmat(f'{PROJECT_ROOT}\\data\\TD_OPF\\ds_data\\case533mt_hi_ds.mat')
...
def case136ma_ds(root_voltage = 1.0, radial = True, flex_percent = 0.5, flex_rate = 0.3):
    ...
    # ppc["baseMVA"]=10
    # ppc["basekV"] = 13.8
    data = loadmat(f'{PROJECT_ROOT}\\data\\TD_OPF\\ds_data\\case136ma_ds.mat')
    ppc["baseMVA"] = data["mpc"]["baseMVA"][0, 0][0, 0]
    ppc["branch"] = data["mpc"]["branch"][0, 0]
    if radial:
        ppc["branch"] = ppc["branch"][ppc["branch"][:,10]==1,:] #radial
...
def case118zh_ds(root_voltage = 1.0, radial = True, flex_percent = 0.5, flex_rate = 0.3):
    ...
    # ppc["baseMVA"]=10
    # ppc["basekV"] = 13.8
    data = loadmat(f'{PROJECT_ROOT}\\data\\TD_OPF\\ds_data\\case118zh_ds.mat')
    ppc["baseMVA"] = data["mpc"]["baseMVA"][0, 0][0, 0]
    ppc["branch"] = data["mpc"]["branch"][0, 0]
    ppc["branch"][:, 2:4] /= 2
    if radial:
...
def case74_ds(root_voltage = 1.0, radial = True, flex_percent = 0.5, flex_rate = 0.3):
    ...
    # ppc["baseMVA"]=10
    # ppc["basekV"] = 13.8
    data = loadmat(f'{PROJECT_ROOT}\\data\\TD_OPF\\ds_data\\case74_ds.mat')
    ppc["baseMVA"] = data["mpc"]["baseMVA"][0, 0][0, 0]
    ppc["branch"] = data["mpc"]["branch"][0, 0]
    ppc["branch"][:, 2:4] /= 1
    if radial:
...
def case51ga_ds(root_voltage = 1.0, radial = True, flex_percent = 0.5, flex_rate = 0.3):
    ...
    # ppc["baseMVA"]=10
    # ppc["basekV"] = 13.8
    data = loadmat(f'{PROJECT_ROOT}\\data\\TD_OPF\\ds_data\\case51ga_ds.mat')
    ppc["baseMVA"] = data["mpc"]["baseMVA"][0, 0][0, 0]
    ppc["branch"] = data["mpc"]["branch"][0, 0]
    ppc["branch"][:, 2:4] /= 2
    if radial:
...
def case33bw_ds(root_voltage = 1.0, radial = True, flex_percent = 0.5, flex_rate = 0.3):
    ...
    # ppc["baseMVA"]=10
    # ppc["basekV"] = 13.8
    data = loadmat(f'{PROJECT_ROOT}\\data\\TD_OPF\\ds_data\\case33bw_ds.mat')
    ppc["baseMVA"] = data["mpc"]["baseMVA"][0, 0][0, 0]
    ppc["branch"] = data["mpc"]["branch"][0, 0]
    ppc["branch"][:, 2:4] /= 2
    if radial:
...
def case10ba_ds(root_voltage = 1.0, radial = True, flex_percent = 0.5, flex_rate = 0.3):
    ...
    # ppc["baseMVA"]=10
    # ppc["basekV"] = 13.8
    data = loadmat(f'{PROJECT_ROOT}\\data\\TD_OPF\\ds_data\\case10ba_ds.mat')
    ppc["baseMVA"] = data["mpc"]["baseMVA"][0, 0][0, 0]
    ppc["branch"] = data["mpc"]["branch"][0, 0]
    ppc["branch"][:, 2:4] /= 2
    if radial:
...
def DScase_train(casedata, model_type='pretrainnet', plot_flag = True, total_samples=100, noise_range = [-0.05,0.05], batch_size=5,device = 'cpu'):  # 使用单相算例，如case33
    ...
    figure_folder = f'{PROJECT_ROOT}\\results\\ds_proj\\{case_name}\\figures'
    ...
    return {
        ...
        'params': params,
        'result_path': f'{PROJECT_ROOT}\\results\\ds_proj\\{case_name}\\{model_type}_weights.pth',
        'n_train': n_train,
        'metadata': {
            'dscasedata': casedata,
        }
...
if __name__ == '__main__':
    ...
    dsppc = case33bw_ds(root_voltage=1, flex_percent = 0.8, flex_rate = 0.3)
    print(dsppc['gen'])
    # P_target = 0
    # Q_target = 0
    # v_target = 0.90
    ...
    #
    # 运行潮流计算
    results, success = runpf(dsppc, ppopt)
