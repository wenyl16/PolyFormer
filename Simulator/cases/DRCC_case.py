...
def generate_asset_samples(params=None):
    ...
    if params is None:
        params = dict(K=5, N_var=10, N_samples=10,
                      ...
                      data_path=f'{PROJECT_ROOT}/data/DRCC/r_samples.csv')
    ...
    if do_save:
        df_samples.to_csv(data_path+'.csv',index=False)
    return df_samples
...
class DRCCModelBuilder:
    ...
    def build_drcc_train(self, casedata, model_type ='pretrainnet', plot_flag = False, total_samples=100, batch_size=5, device = 'cpu'):
        ...
        result_folder = f'{PROJECT_ROOT}\\results\\DRCC\\{case_name}\\g{group_id}'
        ...
        if model_type.lower() == 'pretrainnet' and plot_flag:
            ...
            os.makedirs(result_folder + f'\\figures/pretrain_process', exist_ok=True)
            plotter.save(result_folder + f'\\figures/pretrain_process/step0{0}.png')
        ...
        def training_callback(errorcalculator, epoch):
            ...
            if model_type.lower() == 'pretrainnet' and plot_flag:
                ...
                plotter.save(result_folder + f'\\figures/pretrain_process/step{epoch}.png')
        # 训练参数配置
        ...
        return {
            ...
            'params': params,
            'result_path': f'{PROJECT_ROOT}\\results\\DRCC\\{case_name}\\g{group_id}\\{model_type}_weights.pth',
            'n_train': n_train,
            'metadata': {
                'dscasedata': casedata,
            }
...
if __name__ == "__main__":
    ...
    # data_path = None
    meta_params = dict(K=N_levels, N_var = N_var, N_samples=N_samples,
                               mean_range=(0.03, 0.04), std_range=(0.01, 0.03),
                               lower=-0.1, upper=0.1,
                                do_save = True,
                               data_path = data_path)
    r_samples= generate_asset_samples(params=meta_params)
    r_samples_test = generate_test_samples(N_test=N_test, params=meta_params)

    # r_samples = pd.read_csv(data_path + '.csv')
    # r_test_samples = pd.read_csv(data_path + '_test.csv')
    # print(r_test_samples)
    ...
    # apx_data = {
    #     0: {'A': np.array([[1.0, 2.0], [-1.0, 1.0]]), 'b': np.array([1.5, 0.5])},
    #     # 1: {'A': np.array([[0.5, 1.5, -0.5]]), 'b': np.array([0.7])},
    # }
    # portfolio.build_apx_model(apx_data=apx_data)
    # print(1)
    # case = portfolio.build_drcc_train(portfolio.group_dataset[0])
    portfolio.build_original_model()
    # portfolio.build_apx_model(apx_data=)
    print(portfolio.solve(is_apx=False, tee = False))
