...
def case_polygon(total_samples = 200, noise_scale=0.1, batch_size=1, model_type = 'pretrainnet',device = 'cpu'):
    """固定参数的原始案例实现"""
    # 固定初始化参数
    A_init = np.array([
        [1, 0], [0, 1], [-1, 0], [0, -1],
...
def case_ellipse(total_samples=100, noise_scale=0.3, batch_size=5, model_type='pretrainnet',device = 'cpu'):
    """椭圆约束案例实现"""
    # 固定初始化参数 (二次型矩阵)
    Sigma_init = np.array([
        [5 / 2, -3 / 2],
