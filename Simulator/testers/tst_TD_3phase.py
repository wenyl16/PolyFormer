...
for dsppc_name in dscases:
    ...
    for tsppc_name in tscases:
        ...
        case = DS_case_3phase.DScase_3phase_train(casedata=dsppc, model_type=model_type)
        # 这块是加入近似：
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        ...
        for tsnode, dscasedata in dscasedata_dict.items():
            ...
            errors.append(DS_case_3phase.disagg_DS_3phase(P_target, Q_target, v_target, dscasedata))
        ...
        model = TD_case.TDcase(tscasedata=tsppc, dscasedata_dict=dscasedata_dict, is_apx=False, is_3phase=True)  # is_apx控制是否为近似
        num_constraints = sum(1 for _ in model.component_data_objects(pyo.Constraint, active=True, descend_into=True))
        num_vars = sum(1 for _ in model.component_data_objects(pyo.Var, active=True, descend_into=True))
...
print(df)
output_path = f'{PROJECT_ROOT}\\results\\ds_proj\\td_results\\{model_type}_3phase.xlsx'
